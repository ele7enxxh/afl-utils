...
def show_info():
    print(clr.CYA + "afl-rcrash " + clr.BRI + "%s" % afl_utils.__version__ + clr.RST + " by %s" % afl_utils.__author__)
    print("Run crash samples collected from afl-fuzz.")
    print("")


def run_samples(num_threads, samples, target_cmd, timeout_secs=60):
    ...
    for i in range(0, num_threads, 1):
        t = AflThread.RunThread(i, timeout_secs, target_cmd, in_queue, out_queue, in_queue_lock, out_queue_lock)
...
particularly useful when combined with '-r' or '-f'.")
    ...
    print_ok("runing %d crash samples..." % num_crashes)
    ...
    invalid_samples, timeout_samples = run_samples(int(args.num_threads), crash_samples, args.target_cmd, int(args.timeout_secs))
